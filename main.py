...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet18',
                    ...
                    ' | '.join(model_names) +
                    ' (default: resnet18)')
parser.add_argument('--nltype', default='nl')
parser.add_argument('--k', default=4, type=int)
parser.add_argument('--nlsize', default=[7, 7, 7], type=int, nargs='+')
parser.add_argument('--dropout', type=float, default=0.2)
parser.add_argument('--nl_mod', default=[2, 2, 1000], nargs='+', type=int)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch in my_models:
        ...
        if 'nlne' in args.arch or 'oct' in args.arch or 'sknet' in args.arch:
            model = model_zoo.__dict__[args.arch](
                nltype=args.nltype, nl_mod=args.nl_mod,
                k=args.k, nlsize=args.nlsize, dropout=args.dropout, use_se=args.use_se)
        else:
            model = model_zoo.__dict__[args.arch]()
        if args.pretrained:
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            t = model.load_state_dict(
                checkpoint['state_dict'], strict=not args.soft_resume)
            print('Missing1:', t[0])
            print('Missing2:', t[1])
